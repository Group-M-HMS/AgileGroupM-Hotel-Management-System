  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        aria-expanded={open}
        className="flex items-center gap-2 rounded-full border-2 border-sand bg-white px-4 py-2 font-jakarta text-meta text-jungle/80 transition-colors hover:border-sage"
      >
        Filters
        {selectedAmenities.size > 0 && (
          <span className="flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[11px] font-semibold text-sand-light">
      ...
      {open && (
        <div className="absolute left-0 z-10 mt-2 flex w-56 flex-col gap-1 rounded-2xl border border-sand bg-white p-3 shadow-soft-lg">
          {availableAmenities.length === 0 ? (
            <p className="px-1 py-1 font-jakarta text-meta text-jungle/50">No filters available</p>
          ) : (
            availableAmenities.map(amenity => {
              ...
              return (
                <label
                  key={amenity}
                  className="flex cursor-pointer items-center gap-2 rounded-lg px-1 py-1.5 font-jakarta text-meta text-jungle hover:bg-sand-light"
                >
                  <input
                    type="checkbox"
                    checked={checked}
  ...
  return (
    <div className="flex flex-wrap items-center gap-4">
      <div className="flex items-center gap-2">
        <label htmlFor="sort-by" className="font-jakarta text-meta text-jungle/70">
          Sort by
        </label>
